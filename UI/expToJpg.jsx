/**
 * .jsx for Adobe Illustrator. Marat Shagiev. WebStorm.
 изменение:
 при нажатии cancel, esc или кнопки закрытия окна происходит завершение работы скрипта

 todo: фокус пропадает
 */
...
function expToJpg ( o, w, j ) {
  ...
  if ( File ( fullPath ).exists && mainWin.replaceYes ) {
    yesToReplace = confirm (
      'Replace file ' + spaceOnDash ( w.doc.name.slice ( 0, -(getFileExt ().length) ) ) + mainWin.suffix + '.jpg' + '?'
    );
  }
  if ( yesToReplace ) {
