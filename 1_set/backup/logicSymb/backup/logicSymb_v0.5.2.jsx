...
+function () {
  ...
  (function makeDebugButton () {
    ...
    p.debugButton.onClick = function () {
      ...
      //bt.body = getDocEditCase.toString() + ";" + "var a = getDocEditCase();
      // alert(getDocEditCase.modeDescription(a))";
      //   bt.body = "alert(selection.characters[0].contents)";
//      bt.body = "alert(selection.textRanges.length)";
      bt.body = "alert(selection.characters[0].contents.charCodeAt(0))";
      bt.send ();
    }
  }) ();
  ...
  (function setHendlersToButtons () {
    for ( var i = 0; i < symb.getSymbArr ().length; i++ ) {
      ...
      btns[ i ].onClick = function () {
        var bt = new BridgeTalk ();
        bt.target = 'illustrator';
        var sign = fontList.selection.text;
        bt.body =
          getDocEditCase.toString () + ";" + addSign.toString () + ";" + "addSign(\"" + this.text + "\",\"" + sign + "\")";
        bt.send ();
      }
    }
  }) ();
...
// записать шрифты с поддержкой всех символов логических операций в массив
  ...
  function addSymbol () {
    ...
    /* function _addSymb () {
     ...
     selection.characters[ i ].characterAttributes.textFont = textFonts.getByName ( fontList.selection.text );
     }
