/**
 ...
 refactoring code by Marat Shagiev 20250102
 ----------------------------------------------------------------------------
 */
//@target illustrator
try {
...
function main() {
 ...
 var selection = ad.selection;
 var iniFullPath = Folder.userData + '/illocr.ini';
 var illOCRFile = new File(iniFullPath);
 var colorRGBWhite = new RGBColor(); // белый цвет для щита и окошка
 colorRGBWhite.red = 255;
 colorRGBWhite.green = 255;
 colorRGBWhite.blue = 255;
 ...
 if (!ocrStyle) // Создание стиля абзаца OCR_STYLE, если он не существует
 {
  ...
  ocrStyle = ad.paragraphStyles.add(styleName);
...
// Проходим по всем выделенным объектам
  for (var i = 0; i < ad.selection.length; i++) {
   var selectedItem = ad.selection[i];
  ...
  var currentArtboard = ad.artboards[ad.artboards.getActiveArtboardIndex()]; // Получаем текущий артборд
  ...
  var rectArt = ad.pathItems.rectangle(artboardRect[1], artboardRect[0], widthAR, heightAR); // Создаем прямоугольник с размерами артборда
  rectArt.fillColor = colorRGBWhite; // заливка белая
  var widthTmp = (maxX - minX); // ширина "окошка" для распознования
  var heighTmp = (minY - maxY); // высота "окошка" для распознования
  var rectTmp = ad.pathItems.rectangle(minY, minX, widthTmp, heighTmp); // прямоугольник окошка
  rectTmp.fillColor = colorRGBWhite;
  ...
  ad.selection = [rectTmp, rectArt]; // выделяем окошко и прямоугольник размером с артбоард

  var group = ad.groupItems.add(); //группируем выделенные окошко и щит
  for (var i = 0; i < ad.selection.length; i++)
   ad.selection[i].move(group, ElementPlacement.INSIDE);
  ad.selection = group; // выделяем группу "окошко" и "щит"
  ...
  ad.exportFile(file, ExportType.PNG24, options);    // экспорт файла в формате png с указанными опциями по указанному пути
...
// Удаление рабочих файлов
  ...
  ad.selection.myshild; // выделение щита с окошком
  app.executeMenuCommand('cut'); // удаление щита с окошком

// Создание текстового фрейма и преобразование его как "Area Type"
  var rect = ad.pathItems.rectangle(minY, minX, maxX - minX + 3, minY - maxY); // расширяем на 3 от выделенного объекта
  var textFrame = ad.textFrames.areaText(rect);
  var myStyle = ad.paragraphStyles.getByName("OCR_STYLE"); // присвоение переменной имени стиля абзаца
