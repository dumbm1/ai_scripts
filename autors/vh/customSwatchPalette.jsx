 ...
 *
 * Vasily Hall
 * https://forums.adobe.com/message/6028136#6028136
 * */
...
function swatchPalette() {
  ...
  function getXmlSwatchesXML(swatchArr) {
    ...
    for (var i = 0; i < swatchArr.length; i++) {
      ...
      xmlBody.appendChild(new XML("<swatch></swatch>"));
      xmlBody.swatch[i].order = i + 1;
  ...
  function getAISwatchColor(processColorOrSpot, name) {
    ...
    if (clr == "[RGBColor]") {
      result.type     = "RGB";
      result.colorArr = [round2(clr.red), round2(clr.green), round2(clr.blue)];
      result.uiColor  = [clr.red / 255, clr.green / 255, clr.blue / 255];
      result.plating  = 'Process';
    } else if (clr == "[CMYKColor]") {
      result.type     = "CMYK";
      result.colorArr = [round2(clr.cyan), round2(clr.magenta), round2(clr.yellow), round2(clr.black)];
      result.uiColor  = function() {
      ...
      result.type     = "GRAY";
      result.colorArr = [round2(clr.gray)];
      result.uiColor  = function() {
      ...
      if (clr.spotKind == SpotColorKind.SPOTLAB) {
        ...
        result.type     = "RGB";
        result.colorArr = intClr;
        result.uiColor  = [intClr[0] / 255, intClr[1] / 255, intClr[2] / 255];
  ...
  function btnSkin(elem, aClr, w, h) {
    var g     = elem.graphics;
    var brush = g.newBrush(g.BrushType.SOLID_COLOR, aClr);
    g.newPath();
    g.moveTo(1, 1);
    g.lineTo(w - 1, 1);
    g.lineTo(w - 1, h - 1);
    ...
    brush = g.newBrush(g.BrushType.SOLID_COLOR, ltr);
    g.newPath();
    g.rectPath(0, 0, w, h / 2);
    g.fillPath(brush);
  ...
  function createAltDisp(swObj, uiParent) {
    ...
    for (var m = 0; m < modes.length; m++) {
      if (swObj.colorComp.typename != modes[m]) {
        var altG           = uiParent.add('panel', undefined, modes[m]);
        altG.margins       = [2, 6, 2, 2];
        altG.size          = [80, 88];
        altG.spacing       = 2;
        altG.alignChildren = 'left';
        if (modes[m] == "RGB") {
  ...
  function swatchInfoBox(swObj, uiParent) {
    ...
    var g_1          = baseG.add("panel", undefined, "Swatch Info");
    g_1.size         = [340, 340];
    g_1.spacing      = 2;
    g_1.margins      = [4, 12, 4, 4];
    var nameD        = g_1.add('edittext', undefined, swObj.name, {readonly: true});
    nameD.characters = 20;
    nameD.justify    = 'center';
    if (swObj.name != 'none') {
      var mode           = swObj.colorComp.typename;
      var sep            = g_1.add('panel');
      sep.size           = [310, 2];
      var g_1_0          = g_1.add('group');
      var txtG           = g_1_0.add('group');
      txtG.orientation   = 'column';
      txtG.spacing       = 2;
      txtG.margins       = [2, 2, 2, 2];
      txtG.alignChildren = 'left';
      ...
      var swD  = g_1_0.add('panel');
      swD.size = [34, 34];
      swD.setBg(swObj.fillColor);
      var sep_1     = g_1.add('panel');
      sep_1.size    = [310, 2];
      var g_1_1     = g_1.add('group');
      g_1_1.spacing = 2;
      if (mode == "RGB") {
      ...
      var sep_2              = g_1.add('panel');
      sep_2.size             = [310, 2];
      var originDocG         = g_1.add('group');
      originDocG.orientation = 'row';
      var originDocH         = originDocG.add('statictext', undefined, "Originating Document");
      var originDocE         = originDocG.add('edittext', undefined, swObj.info.originDoc, {readonly: true});
      originDocE.characters  = 18;
      var g_1_3              = g_1.add('panel', undefined, 'Notes');
      g_1_3.margins          = [2, 8, 2, 2];
      g_1_3.spacing          = 2;
      var notesE             = g_1_3.add('edittext', undefined, '', {readonly: true, multiline: true});
      notesE.size            = [310, 60];
      ...
      viewBigBtn.onClick = function() {
        if (notesE.text != '') {
          var notesW     = new Window('dialog', "Notes in a bigger window:");
          notesW.margins = [4, 4, 4, 4];
          var bignotesE  = notesW.add('edittext', undefined, notesE.text, {multiline: true, readonly: true});
          bignotesE.size = [300, 360];
      ...
      editBtn.onClick    = function() {
        var editResult = function() {
          ...
          var editE     = w.add('edittext', undefined, notesE.text, {multiline: true});
          editE.size    = [310, 160];
          editE.active  = true;
  ...
  function infoWindow(SESSION) { //======================================MINI UI
    ...
    var w           = new Window('dialog', "Swatch Info & Preferences");
    w.margins       = [4, 4, 4, 4];
    var g_0         = w.add('group');
    g_0.orientation = 'column';
    g_0.spacing     = 4;
    ...
    var g_0_1         = g_0.add("panel", undefined, "Main Window Prefrences");
    g_0_1.size        = [340, 44];
    g_0_1.orientation = 'column';
    var boxGrp        = g_0_1.add('group');
    var modelBx       = boxGrp.add('checkbox', undefined, "Show Color Mode");
    modelBx.value     = SESSION.options.showMode;
    var noteBx        = boxGrp.add('checkbox', undefined, "Show Note Marker");
    noteBx.value      = SESSION.options.showNotes;
    var g_0_2         = g_0.add("panel", undefined, "Persistence Options");
    g_0_2.size        = [340, 54];
    var saveBtn       = g_0_2.add('button', undefined, "Save Current Session");
    var okBtn         = w.add('button', undefined, 'Ok');
    okBtn.size        = [340, 20];
    okBtn.margins     = [0, 0, 0, 0];
    okBtn.onClick     = function() {
      w.close();
    }
  ...
  function bt_func_rt(objString, args) { //puts swatches into palette.
    var docSws  = eval(objString).swatches;
    var SESSION = args[0];
    for (var s = 0; s < docSws.length; s++) {
      ...
      if (SESSION.selection[s] != undefined) {
        if (thisSw.name != 'none') {
          ...
          var info = {
            plating: thisSw.plating, originDoc: thisSw.originDoc,
  ...
  function ShowPalette(name) {
    ...
    g_0.margins                 = [1, 1, 1, 1];
    g_0.spacing                 = 1;
    var g_0_1                   = g_0.add('panel', undefined, '', {borderStyle: 'sunken'});
    g_0_1.size                  = [140, 160];
    g_0_1.margins               = [0, 1, 1, 0];
    g_0_1.orientation           = 'row';
    g_0_1.spacing               = 1;
    g_0_1.alignChildren         = 'top';
    var swatchContainer         = g_0_1.add('group');
    swatchContainer.spacing     = 1;
    swatchContainer.size        = [123, 260];
    swatchContainer.margins     = [0, 0, 0, 0];
    ...
    for (var i = 0; i < 50; i++) {// Setting up all 50 swatch slots.
      var swP        = swatchContObj[('swG_' + Math.floor((i / 5)))].add('group');
      swP.size       = [24, 24];
      swP.order      = i;
      ...
      thisSwatch.elem.addEventListener('mousedown', function(ev) {
        ...
        if (keyState.ctrlKey == false && keyState.shiftKey == false && keyState.metaKey == false && keyState.altKey == false) {
          ...
          var infoDisp = function() {
            var w     = new Window('dialog', subj.name + " Swatch Info");
            w.margins = [4, 8, 4, 4];
            w.spacing = 2;
        ...
        if (SESSION.selection.length > 1) {
          nameD.text            = "--Multi-Selection: " + SESSION.selection.length + "--";
          SESSION.lastSelection = SESSION.selection[0];
        } else if (SESSION.selection.length == 1) {
          nameD.text            = SESSION.selection[0].name;
          SESSION.lastSelection = SESSION.selection[0];
    ...
    var g_0_1_scrl  = g_0_1.add('scrollbar');
    g_0_1_scrl.size = [10, 152];
    var g_0_2       = g_0.add('group');
    g_0_2.indent    = 1;
    var nameD       = g_0_2.add('edittext', undefined, '', {readonly: true});
    nameD.size      = [140, 20];
    nameD.setBg([1, 1, 1]);
    SESSION.nameDisplay   = nameD;
    var btnG              = g_0.add('group');
    btnG.margins          = [1, 1, 1, 1];
    btnG.spacing          = 2;
    var infoBtn           = btnG.add('button', undefined, 'I');
    infoBtn.size          = [26, 20];
    ...
    var addToDocBtn       = btnG.add('button', undefined, 'T');
    addToDocBtn.size      = [26, 20];
    ...
    var addBtn            = btnG.add('button', undefined, 'A');
    addBtn.size           = [26, 20];
    ...
    var rmvBtn            = btnG.add('button', undefined, 'R');
    rmvBtn.size           = [26, 20];
    ...
    var btnG_2            = g_0.add('group');
    btnG_2.margins        = [1, 1, 1, 1];
    btnG_2.spacing        = 2;
    var rstBtn            = btnG_2.add('button', undefined, 'Reset');
    rstBtn.size           = [65, 20];
    ...
    var okBtn             = btnG_2.add('button', undefined, 'Close');
    okBtn.size            = [65, 20];
        ...
        if (allSwatchBoxes[i] && swXmlArr[i].name != 'none') {
          ...
          initSwatch(SESSION, thisSw, thisInfo.name, thisInfo.colorComp.mode,
            thisInfo.colorComp.comp, getUIRGB(colorMode, colorArr),
            {
              ...
              altClrs:   {
                rgbClr:  thisSw.rgbClr,
                cmykClr: thisSw.cmykClr,
                labClr:  thisSw.labClr,
                grayClr: thisSw.grayClr
              },
